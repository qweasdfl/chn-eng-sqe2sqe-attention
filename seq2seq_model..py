"""
工程目录注解：
Chinese-datapre.py  实现了数据的预处理和数据集的准备
evaluate.py  定义了一些评估方法，用于测试
seq2seq_model.py  搭建了encoder和attention decoder，均采用GRU实现
test.py 加载训练好的模型，实现翻译
train.py  顾名思义就是训练脚本
util.py    包含了一些用到的工具函数，如绘图等
...
def get_datas(file = "data\\translate.csv",nums = None):
...
if __name__ == "__main__":
    ...
    en_datas,ch_datas = get_datas(nums=1000)
